...
def benchmark(problems, output_csv):
    ...
    with open(output_csv, "a") as results:
        ...
        for problem_name, topo_fname, tm_fname in problems:
            ...
            if problem.traffic_matrix.scale_factor != 64:
                continue
...
###         
            NUM_PARTITION_SWEEP = [2,4,8,16,32]
            # specify the partitioner to be LeaderElection
            for num_partitions_to_set in NUM_PARTITION_SWEEP:
                partitioner = LeaderElection(num_partitions_to_set)
###
...
###
                        ...
                        # zero out cross-partition commodities and edge capacity
                        for p in range(num_partitions_to_set):
                            for u in problem.G.nodes:
                                for v in problem.G.nodes:
                                    if partition_vector[u] != partition_vector[v]:
                                        problem.traffic_matrix.tm[u,v] = 0
                                        if (u,v) in problem.G.edges:
                                            problem.G[u][v]['capacity'] = 0
###                    
...
###
                        # write sol dict
                        total_sol_dict = ncflow.sol_dict
                        with open(
                            log.name.replace(".txt", "-total-sol-dict.pkl"),
                            "wb",
                        ) as w:
                            pickle.dump(total_sol_dict, w)
###
